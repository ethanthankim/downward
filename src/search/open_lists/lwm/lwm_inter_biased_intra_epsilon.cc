#include "lwm_inter_biased_intra_epsilon.h"
...
class LWMInterBiasedIntraEpOpenList : public OpenList<Entry> {
...
public:
    explicit LWMInterBiasedIntraEpOpenList(const plugins::Options &opts);
    virtual ~LWMInterBiasedIntraEpOpenList() override = default;
...
template<class Entry>
void LWMInterBiasedIntraEpOpenList<Entry>::notify_initial_state(const State &initial_state) {
    cached_parent_type_bucket_index = -1;
    cached_parent_type_h = INT32_MAX;
...
template<class Entry>
void LWMInterBiasedIntraEpOpenList<Entry>::notify_state_transition(
...
// template<class Entry>
// bool LWMInterBiasedIntraEpOpenList<Entry>::node_at_index_1_bigger(const int& index1, const int& index2) {
...
template<class Entry>
void LWMInterBiasedIntraEpOpenList<Entry>::do_insertion(
    EvaluationContext &eval_context, const Entry &entry) {
    
    int new_h = eval_context.get_evaluator_value_or_infinity(evaluator.get());
    int type_def_index;

    if (new_h < cached_parent_type_h) { 
...
template<class Entry>
void LWMInterBiasedIntraEpOpenList<Entry>::adjust_node_up(vector<TypeNode> &heap, size_t pos) {
...
template<class Entry>
Entry LWMInterBiasedIntraEpOpenList<Entry>::remove_min() {
    ...
    while(true) {
        ...
        TypeBucket &bucket = buckets[bucket_index];
        ...
        if (rng->random() <= intra_e) {
            ...
            bucket.entries[pos].h = numeric_limits<int>::min();
            adjust_node_up(bucket.entries, pos);
        }
...
template<class Entry>
LWMInterBiasedIntraEpOpenList<Entry>::LWMInterBiasedIntraEpOpenList(const plugins::Options &opts)
...
template<class Entry>
bool LWMInterBiasedIntraEpOpenList<Entry>::empty() const {
    return type_buckets.empty();
}

template<class Entry>
void LWMInterBiasedIntraEpOpenList<Entry>::clear() {
    type_buckets.clear();
    type_defs.clear();
}

template<class Entry>
bool LWMInterBiasedIntraEpOpenList<Entry>::is_dead_end(
    EvaluationContext &eval_context) const {
    return eval_context.is_evaluator_value_infinite(evaluator.get());
}

template<class Entry>
bool LWMInterBiasedIntraEpOpenList<Entry>::is_reliable_dead_end(
    EvaluationContext &eval_context) const {
    return is_dead_end(eval_context) && evaluator->dead_ends_are_reliable();
}

template<class Entry>
void LWMInterBiasedIntraEpOpenList<Entry>::get_path_dependent_evaluators(
    set<Evaluator *> &evals) {
    evaluator->get_path_dependent_evaluators(evals);
}

LWMInterBiasedIntraEpOpenListFactory::LWMInterBiasedIntraEpOpenListFactory(
    const plugins::Options &options)
    : options(options) {
}

unique_ptr<StateOpenList>
LWMInterBiasedIntraEpOpenListFactory::create_state_open_list() {
    return utils::make_unique_ptr<LWMInterBiasedIntraEpOpenList<StateOpenListEntry>>(options);
}

unique_ptr<EdgeOpenList>
LWMInterBiasedIntraEpOpenListFactory::create_edge_open_list() {
    return utils::make_unique_ptr<LWMInterBiasedIntraEpOpenList<EdgeOpenListEntry>>(options);
}

class LWMInterBiasedIntraEpOpenListFeature : public plugins::TypedFeature<OpenListFactory, LWMInterBiasedIntraEpOpenListFactory> {
public:
    LWMInterBiasedIntraEpOpenListFeature() : TypedFeature("lwm_inter_biased_intra_epsilon") {
...
static plugins::FeaturePlugin<LWMInterBiasedIntraEpOpenListFeature> _plugin;
}