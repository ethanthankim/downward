#include "type_lwm_intra_biased_open_list.h"
...
class LWMIntraBiasedOpenList : public OpenList<Entry> {
...
public:
    explicit LWMIntraBiasedOpenList(const plugins::Options &opts);
    virtual ~LWMIntraBiasedOpenList() override = default;
...
template<class Entry>
void LWMIntraBiasedOpenList<Entry>::notify_state_transition(
...
template<class Entry>
bool LWMIntraBiasedOpenList<Entry>::node_at_index_1_bigger(const int& index1, const int& index2) {
...
template<class Entry>
void LWMIntraBiasedOpenList<Entry>::do_insertion(
...
template<class Entry>
Entry LWMIntraBiasedOpenList<Entry>::remove_min() {
    int type_index;
    do {
...
template<class Entry>
LWMIntraBiasedOpenList<Entry>::LWMIntraBiasedOpenList(const plugins::Options &opts)
...
template<class Entry>
bool LWMIntraBiasedOpenList<Entry>::empty() const {
    return type_buckets.empty();
}

template<class Entry>
void LWMIntraBiasedOpenList<Entry>::clear() {
    type_buckets.clear();
    all_nodes.clear();
}

template<class Entry>
bool LWMIntraBiasedOpenList<Entry>::is_dead_end(
    EvaluationContext &eval_context) const {
    return eval_context.is_evaluator_value_infinite(evaluator.get());
}

template<class Entry>
bool LWMIntraBiasedOpenList<Entry>::is_reliable_dead_end(
    EvaluationContext &eval_context) const {
    return is_dead_end(eval_context) && evaluator->dead_ends_are_reliable();
}

template<class Entry>
void LWMIntraBiasedOpenList<Entry>::get_path_dependent_evaluators(
    set<Evaluator *> &evals) {
    evaluator->get_path_dependent_evaluators(evals);
}

LWMIntraBiasedOpenListFactory::LWMIntraBiasedOpenListFactory(
    const plugins::Options &options)
    : options(options) {
}

unique_ptr<StateOpenList>
LWMIntraBiasedOpenListFactory::create_state_open_list() {
    return utils::make_unique_ptr<LWMIntraBiasedOpenList<StateOpenListEntry>>(options);
}

unique_ptr<EdgeOpenList>
LWMIntraBiasedOpenListFactory::create_edge_open_list() {
    return utils::make_unique_ptr<LWMIntraBiasedOpenList<EdgeOpenListEntry>>(options);
}

class LWMIntraBiasedOpenListFeature : public plugins::TypedFeature<OpenListFactory, LWMIntraBiasedOpenListFactory> {
public:
    LWMIntraBiasedOpenListFeature() : TypedFeature("lwm_intra_biased") {
...
static plugins::FeaturePlugin<LWMIntraBiasedOpenListFeature> _plugin;
}