        ...
        int eval) 
{
    if (new_partition || partition_to_id_pair.count(partition_key)==0) {
        ...
        node_to_prog.emplace(node_key, max(cached_parent_depth - 1, 1));
...
public:
    InterBiasedProgressPolicyFeature() : TypedFeature("inter_biased_prog") {
        document_subcategory("partition_policies");
        document_title("Biased progress partition selection");
        document_synopsis(
            "Choose the next partition biased in favour of high progress partitions.");
        add_option<bool>(
            "pref_only",
