...
InterBiasedLevelPolicy::InterBiasedLevelPolicy(const plugins::Options &opts)
...
// void InterBiasedLevelPolicy::verify_heap() {
...
void InterBiasedLevelPolicy::notify_partition_transition(int parent_part, int child_part) {
    ...
    cached_parent_lwm = partition_to_id_pair.at(cached_parent_part).first;
}
...
int InterBiasedLevelPolicy::get_next_partition() {
    // int count_i = 0;
    // total_gets+=1;
    // if (total_gets % 100 == 0) {
    //     verify_heap();
...
void InterBiasedLevelPolicy::notify_insert(
        int partition_key,
        int node_key,
        bool new_partition,
        int eval) 
...
InterBiasedLevelPolicy::PartitionNode InterBiasedLevelPolicy::remove_partition(int partition_key) {
    auto partition_ids = partition_to_id_pair.at(partition_key); 
...
void InterBiasedLevelPolicy::notify_removal(int partition_key, int node_key) {
    auto& partition_ids = partition_to_id_pair.at(partition_key);
...
class InterBiasedLevelPolicyFeature : public plugins::TypedFeature<PartitionPolicy, InterBiasedLevelPolicy> {
public:
    InterBiasedLevelPolicyFeature() : TypedFeature("inter_biased_level") {
        document_subcategory("partition_policies");
        document_title("Biased level partition selection");
        document_synopsis(
            "Choose the next partition biased in favour of low level partitions.");
        add_option<bool>(
            "pref_only",
...
static plugins::FeaturePlugin<InterBiasedLevelPolicyFeature> _plugin;
}