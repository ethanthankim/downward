#include "type_based_path_open_list.h"
...
class TypeBasedPathOpenList : public OpenList<Entry> {
    ...
    PerStateInformation<vector<int>> state_eval_cache;
    vector<int> eval_cache;
    int parent_type;
...
public:
    explicit TypeBasedPathOpenList(const plugins::Options &opts);
    virtual ~TypeBasedPathOpenList() override = default;
...
template<class Entry>
void TypeBasedPathOpenList<Entry>::notify_state_transition(
...
template<class Entry>
void TypeBasedPathOpenList<Entry>::do_insertion(
    ...
    auto tmp = std::vector<int>(eval_cache.begin(), eval_cache.end()-1);
    if (key == tmp) {
        key.push_back(parent_type);
    } else {
        key.push_back(eval_context.get_state().get_id().get_value());
    }
    state_eval_cache[eval_context.get_state()] = key;
...
template<class Entry>
TypeBasedPathOpenList<Entry>::TypeBasedPathOpenList(const plugins::Options &opts)
    : rng(utils::parse_rng_from_options(opts)),
      evaluators(opts.get_list<shared_ptr<Evaluator>>("evaluators")) {
}

template<class Entry>
Entry TypeBasedPathOpenList<Entry>::remove_min() {
...
template<class Entry>
bool TypeBasedPathOpenList<Entry>::empty() const {
    return keys_and_buckets.empty();
}

template<class Entry>
void TypeBasedPathOpenList<Entry>::clear() {
    keys_and_buckets.clear();
    key_to_bucket_index.clear();
}

template<class Entry>
bool TypeBasedPathOpenList<Entry>::is_dead_end(
...
template<class Entry>
bool TypeBasedPathOpenList<Entry>::is_reliable_dead_end(
    EvaluationContext &eval_context) const {
    for (const shared_ptr<Evaluator> &evaluator : evaluators) {
...
template<class Entry>
void TypeBasedPathOpenList<Entry>::get_path_dependent_evaluators(
    set<Evaluator *> &evals) {
    for (const shared_ptr<Evaluator> &evaluator : evaluators) {
...
TypeBasedPathOpenListFactory::TypeBasedPathOpenListFactory(
    const plugins::Options &options)
    : options(options) {
}

unique_ptr<StateOpenList>
TypeBasedPathOpenListFactory::create_state_open_list() {
    return utils::make_unique_ptr<TypeBasedPathOpenList<StateOpenListEntry>>(options);
}

unique_ptr<EdgeOpenList>
TypeBasedPathOpenListFactory::create_edge_open_list() {
    return utils::make_unique_ptr<TypeBasedPathOpenList<EdgeOpenListEntry>>(options);
}

class TypeBasedPathOpenListFeature : public plugins::TypedFeature<OpenListFactory, TypeBasedPathOpenListFactory> {
public:
    TypeBasedPathOpenListFeature() : TypedFeature("type_based_path") {
    ...
    virtual shared_ptr<TypeBasedPathOpenListFactory> create_component(const plugins::Options &options, const utils::Context &context) const override {
        plugins::verify_list_non_empty<shared_ptr<Evaluator>>(context, options, "evaluators");
        return make_shared<TypeBasedPathOpenListFactory>(options);
    }
};

static plugins::FeaturePlugin<TypeBasedPathOpenListFeature> _plugin;
}
