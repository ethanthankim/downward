...
class BTSInterBiasedIntraEpOpenList : public OpenList<Entry> {
    ...
    unordered_map<int, TypeDef> type_defs;
...
public:
    explicit BTSInterBiasedIntraEpOpenList(const plugins::Options &opts);
    virtual ~BTSInterBiasedIntraEpOpenList() override = default;
...
template<class Entry>
void BTSInterBiasedIntraEpOpenList<Entry>::notify_initial_state(const State &initial_state) {
...
template<class Entry>
void BTSInterBiasedIntraEpOpenList<Entry>::notify_state_transition(
...
// template<class Entry>
// bool BTSInterBiasedIntraEpOpenList<Entry>::node_at_index_1_bigger(const int& index1, const int& index2) {
...
template<class Entry>
void BTSInterBiasedIntraEpOpenList<Entry>::do_insertion(
...
template<class Entry>
void BTSInterBiasedIntraEpOpenList<Entry>::adjust_node_up(vector<TypeNode> &heap, size_t pos) {
...
template<class Entry>
Entry BTSInterBiasedIntraEpOpenList<Entry>::remove_min() {
...
template<class Entry>
BTSInterBiasedIntraEpOpenList<Entry>::BTSInterBiasedIntraEpOpenList(const plugins::Options &opts)
...
template<class Entry>
bool BTSInterBiasedIntraEpOpenList<Entry>::empty() const {
    return type_buckets.empty();
}

template<class Entry>
void BTSInterBiasedIntraEpOpenList<Entry>::clear() {
    type_buckets.clear();
    type_defs.clear();
}

template<class Entry>
bool BTSInterBiasedIntraEpOpenList<Entry>::is_dead_end(
    EvaluationContext &eval_context) const {
    return eval_context.is_evaluator_value_infinite(evaluator.get());
}

template<class Entry>
bool BTSInterBiasedIntraEpOpenList<Entry>::is_reliable_dead_end(
    EvaluationContext &eval_context) const {
    return is_dead_end(eval_context) && evaluator->dead_ends_are_reliable();
}

template<class Entry>
void BTSInterBiasedIntraEpOpenList<Entry>::get_path_dependent_evaluators(
    set<Evaluator *> &evals) {
    evaluator->get_path_dependent_evaluators(evals);
}

BTSInterBiasedIntraEpOpenListFactory::BTSInterBiasedIntraEpOpenListFactory(
    const plugins::Options &options)
    : options(options) {
}

unique_ptr<StateOpenList>
BTSInterBiasedIntraEpOpenListFactory::create_state_open_list() {
    return utils::make_unique_ptr<BTSInterBiasedIntraEpOpenList<StateOpenListEntry>>(options);
}

unique_ptr<EdgeOpenList>
BTSInterBiasedIntraEpOpenListFactory::create_edge_open_list() {
    return utils::make_unique_ptr<BTSInterBiasedIntraEpOpenList<EdgeOpenListEntry>>(options);
}

class BTSInterBiasedIntraEpOpenListFeature : public plugins::TypedFeature<OpenListFactory, BTSInterBiasedIntraEpOpenListFactory> {
public:
    BTSInterBiasedIntraEpOpenListFeature() : TypedFeature("bts_inter_biased_intra_epsilon") {
...
static plugins::FeaturePlugin<BTSInterBiasedIntraEpOpenListFeature> _plugin;
}