#include "type_bts_intra_percolation_open_list.h"
...
class BTSIntraPercolationOpenList : public OpenList<Entry> {
...
public:
    explicit BTSIntraPercolationOpenList(const plugins::Options &opts);
    virtual ~BTSIntraPercolationOpenList() override = default;
...
template<class Entry>
void BTSIntraPercolationOpenList<Entry>::notify_initial_state(const State &initial_state) {
...
template<class Entry>
void BTSIntraPercolationOpenList<Entry>::notify_state_transition(
...
template<class Entry>
bool BTSIntraPercolationOpenList<Entry>::node_at_index_1_bigger(const int& index1, const int& index2) {
...
template<class Entry>
void BTSIntraPercolationOpenList<Entry>::do_insertion(
...
template<class Entry>
Entry BTSIntraPercolationOpenList<Entry>::remove_min() {
    int type_index;
    do {
...
template<class Entry>
BTSIntraPercolationOpenList<Entry>::BTSIntraPercolationOpenList(const plugins::Options &opts)
    : rng(utils::parse_rng_from_options(opts)),
      evaluator(opts.get<shared_ptr<Evaluator>>("eval")) {
}

template<class Entry>
bool BTSIntraPercolationOpenList<Entry>::empty() const {
    return type_heaps.empty();
}

template<class Entry>
void BTSIntraPercolationOpenList<Entry>::clear() {
    type_heaps.clear();
    all_nodes.clear();
}

template<class Entry>
bool BTSIntraPercolationOpenList<Entry>::is_dead_end(
    EvaluationContext &eval_context) const {
    return eval_context.is_evaluator_value_infinite(evaluator.get());
}

template<class Entry>
bool BTSIntraPercolationOpenList<Entry>::is_reliable_dead_end(
    EvaluationContext &eval_context) const {
    return is_dead_end(eval_context) && evaluator->dead_ends_are_reliable();
}

template<class Entry>
void BTSIntraPercolationOpenList<Entry>::get_path_dependent_evaluators(
    set<Evaluator *> &evals) {
    evaluator->get_path_dependent_evaluators(evals);
}

BTSIntraPercolationOpenListFactory::BTSIntraPercolationOpenListFactory(
    const plugins::Options &options)
    : options(options) {
}

unique_ptr<StateOpenList>
BTSIntraPercolationOpenListFactory::create_state_open_list() {
    return utils::make_unique_ptr<BTSIntraPercolationOpenList<StateOpenListEntry>>(options);
}

unique_ptr<EdgeOpenList>
BTSIntraPercolationOpenListFactory::create_edge_open_list() {
    return utils::make_unique_ptr<BTSIntraPercolationOpenList<EdgeOpenListEntry>>(options);
}

class BTSIntraPercolationOpenListFeature : public plugins::TypedFeature<OpenListFactory, BTSIntraPercolationOpenListFactory> {
public:
    BTSIntraPercolationOpenListFeature() : TypedFeature("bts_intra_percolation") {
...
static plugins::FeaturePlugin<BTSIntraPercolationOpenListFeature> _plugin;
}