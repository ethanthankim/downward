...
class BTSOpenList : public OpenList<Entry> {
...
public:
    explicit BTSOpenList(const plugins::Options &opts);
    virtual ~BTSOpenList() override = default;
...
template<class Entry>
void BTSOpenList<Entry>::notify_initial_state(const State &initial_state) {
    cached_parent_info = {INT32_MAX, -1};
}

template<class Entry>
void BTSOpenList<Entry>::notify_state_transition(
    const State &parent_state, OperatorID op_id, const State &state) {
    cached_parent_info = state_type_infos[parent_state];
}

template<class Entry>
BTSOpenList<Entry>::TypeInfo BTSOpenList<Entry>::insert_type_info(EvaluationContext &eval_context) {
...
template<class Entry>
void BTSOpenList<Entry>::do_insertion(
...
template<class Entry>
Entry BTSOpenList<Entry>::remove_min() {
...
template<class Entry>
BTSOpenList<Entry>::BTSOpenList(const plugins::Options &opts)
    : rng(utils::parse_rng_from_options(opts)),
      evaluator(opts.get<shared_ptr<Evaluator>>("eval")) {
}

template<class Entry>
bool BTSOpenList<Entry>::empty() const {
    return keys_and_buckets.empty();
}

template<class Entry>
void BTSOpenList<Entry>::clear() {
    keys_and_buckets.clear();
    key_to_bucket_index.clear();
}

template<class Entry>
bool BTSOpenList<Entry>::is_dead_end(
    EvaluationContext &eval_context) const {
    return eval_context.is_evaluator_value_infinite(evaluator.get());
}

template<class Entry>
bool BTSOpenList<Entry>::is_reliable_dead_end(
    EvaluationContext &eval_context) const {
    return is_dead_end(eval_context) && evaluator->dead_ends_are_reliable();
}

template<class Entry>
void BTSOpenList<Entry>::get_path_dependent_evaluators(
    set<Evaluator *> &evals) {
    evaluator->get_path_dependent_evaluators(evals);
}

BTSOpenListFactory::BTSOpenListFactory(
    const plugins::Options &options)
    : options(options) {
}

unique_ptr<StateOpenList>
BTSOpenListFactory::create_state_open_list() {
    return utils::make_unique_ptr<BTSOpenList<StateOpenListEntry>>(options);
}

unique_ptr<EdgeOpenList>
BTSOpenListFactory::create_edge_open_list() {
    return utils::make_unique_ptr<BTSOpenList<EdgeOpenListEntry>>(options);
}

class BTSOpenListFeature : public plugins::TypedFeature<OpenListFactory, BTSOpenListFactory> {
public:
    BTSOpenListFeature() : TypedFeature("bts_type") {
        document_title("Type system to approximate bench transition system (BTS)");
        document_synopsis(
...
static plugins::FeaturePlugin<BTSOpenListFeature> _plugin;
}