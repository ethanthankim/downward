#include "iterated_search_continued.h"
...
IteratedSearchContinued::IteratedSearchContinued(const plugins::Options &opts)
    : SearchEngine(opts),
      engine_configs(opts.get_list<parser::LazyValue>("engine_configs")),
      pass_bound(opts.get<bool>("pass_bound")),
      f_bound(opts.get<bool>("f_bound")),
...
shared_ptr<SearchEngine> IteratedSearchContinued::get_search_engine(
...
shared_ptr<SearchEngine> IteratedSearchContinued::create_current_phase() {
...
SearchStatus IteratedSearchContinued::step() {
...
SearchStatus IteratedSearchContinued::step_return_value() {
...
void IteratedSearchContinued::print_statistics() const {
    log << "Cumulative statistics:" << endl;
    statistics.print_detailed_statistics();
}

void IteratedSearchContinued::save_plan_if_necessary() {
    // We don't need to save here, as we automatically save after
    // each successful search iteration.
}

class IteratedSearchContinuedFeature : public plugins::TypedFeature<SearchEngine, IteratedSearchContinued> {
public:
    IteratedSearchContinuedFeature() : TypedFeature("iterated_continued") {
        document_title("Iterated Continued search");
        document_synopsis("");
        ...
        add_option<bool>(
            "f_bound",
            "use the astar evaluator f=g+h to prune nodes from open."
            "This should only be used with admissible heuristics.",
            "false");
        add_option<bool>(
            "repeat_last",
    ...
    virtual shared_ptr<IteratedSearchContinued> create_component(const plugins::Options &options, const utils::Context &context) const override {
        ...
        */
        ...
        return make_shared<IteratedSearchContinued>(options_copy);
    }
};

static plugins::FeaturePlugin<IteratedSearchContinuedFeature> _plugin;
}